...
if( ! class_exists( 'Beacon' ) ) {
	...
	class Beacon {
		...
		public function load_textdomain() {
			...
			$mofile = sprintf( '%1$s-%2$s.mo', 'beacon', $locale );
			...
			if( file_exists( $mofile_global ) ) {
				...
				load_textdomain( 'beacon', $mofile_global );
			} elseif( file_exists( $mofile_local ) ) {
				// Look in local /wp-content/plugins/beacon/languages/ folder
				load_textdomain( 'beacon', $mofile_local );
			} else {
				// Load the default language files
				load_plugin_textdomain( 'beacon', false, $lang_dir );
			}
		}
	}
}
